...
const OwnerPayments: React.FC = () => {
  ...
  const getUniqueProperties = () => {
    ...
    [...payments, ...receipts].forEach(item => {
      if ('unit' in item && item.unit?.property) {
        properties.add(JSON.stringify({
          id: item.unit.property.id,
          name: item.unit.property.name
        }));
      } else if ('property' in item && item.property) {
        properties.add(JSON.stringify({
          id: item.property.id,
    ...
    return Array.from(properties).map(p => JSON.parse(p as string));
  };
  ...
  const filteredPayments = payments.filter((payment: any) => {
    ...
    const matchesProperty = propertyFilter === 'all' || payment.unit.property.id.toString() === propertyFilter;
  ...
  const filteredReceipts = receipts.filter((receipt: any) => {
    ...
    const receiptPropertyId = receipt.property?.id || receipt.unit?.property?.id;
    const matchesProperty = propertyFilter === 'all' || receiptPropertyId?.toString() === propertyFilter;

    return matchesSearch && matchesPaymentMethod && matchesProperty;
  });
      ...
      <div className="bg-white rounded-lg shadow-sm">
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-semibold text-gray-900">Property Payments</h1>
