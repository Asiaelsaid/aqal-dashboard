...
interface ICountry {
  ...
  interface SelectProps {
    onCategoryChange: (category: ICountry) => void; 
  }

const SelectCountry : React.FC<SelectProps> = ({onCategoryChange}) => {
    const [selected, setSelected] = useState<ICountry>()


    const handleSelection = (category:ICountry) => {
    ...
    <div className="relative mt-2">
      <ListboxButton className="relative w-full cursor-default rounded-md bg-white py-3 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
        <span className="flex items-center">
          <img alt="" src={selected?.imgURL} className="h-7 w-7 flex-shrink-0 rounded-full" />
